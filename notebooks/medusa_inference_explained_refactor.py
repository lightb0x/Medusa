...
from medusa.model.medusa_model_refactor import MedusaModel
...
def medusa_forward(input_ids, model, tokenizer, medusa_choices, temperature, posterior_threshold, posterior_alpha, max_steps = 512):
    ...
    for idx in range(max_steps): 
        with timed(wall_times, 'medusa'):
            medusa_candidates, base_candidate, medusa_candidates_logits = sample_candidates(
                medusa_logits,
                logits,
            ...
            # Generate medusa buffers
            medusa_buffers = medusa_buffers = generate_medusa_buffers(
                medusa_choices, device=model.device
            )
            # Update medusa attn mask
            model.medusa_mask = medusa_buffers["medusa_attn_mask"]

            # Select candidates
            candidates, tree_candidates = select_candidates(
                medusa_candidates,
                base_candidate,
                medusa_buffers["tree_indices"],
                medusa_buffers["retrieve_indices"],
...
model_name = 'FasterDecoding/medusa-v1.0-vicuna-7b-v1.5'
